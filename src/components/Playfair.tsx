...
import { playfairEncode, playfairDecode } from "../utils/helper";
...
export const PlayfairEncode: React.FC = () => {
  ...
  const encode = () => {
    if (plainText.length && key.length) {
      setEncodedText(playfairEncode(plainText, key));
    }
  };
...
export const PlayfairDecode: React.FC = () => {
  const [encodedText, setEncodedText] = useState<string>("");
  const [key, setKey] = useState<string>("");
  ...
  const decode = () => {
    if (encodedText.length && key.length) {
      setDecodedText(playfairDecode(encodedText, key));
    }
  };
